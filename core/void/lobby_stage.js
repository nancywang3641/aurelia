...
(function () {
    ...
    function _openActorMenu(a, cx, cy) {
        _closeActorMenu(); _closeDressRoom(); _closeNpcHistory(); _closeLobbySettings(); _closeTheaterWin();
    ...
    // ── 🎬 小劇場窗口（右上按鈕/頭頂泡泡進入；「正在對話」=當前配對、「回顧」=記事，吸收原「小劇場歷史」的編輯/刪除/重壓縮/清空）──
    ...
    function _renderTwLive(body) {
        ...
        loc.append(document.createTextNode((SCENE_HEADER[S.scene] || {}).badge || ''));
    ...
    function _openTheaterWin(tab) {
        _closeTheaterWin(); _closeDressRoom(); _closeNpcHistory(); _closeLobbySettings();
        const box = document.createElement('div');
...
'【大廳守衛】你的地盤是 E 區殘塔404室（堆滿伺服器與生活垃圾的地下據點）。你曾在虛擬世界寫過數位分身「柴郡」，但你以為它早被白則發現後清掉了；若有人提起 404 或柴郡，你會不在乎地打哈哈帶過、裝作早清掉了，絕不表現出知道柴郡還在運行。輕鬆閒聊為主，不推進正式劇情、不代寫對方的行動。';
    ...
    function _startTheater() {
        ...
        _theaterFace(a, b);
        [a, b].forEach(n => { n.walking = false; if (n.sheet) { n.frame = 1; n.animT = 0; } placeActor(n); });
        const mkIcon = (n) => {
    ...
    function startTalk(npc) {
        if (S.edit) return;
        if (S.theater && (npc === S.theater.a || npc === S.theater.b)) _endTheater();   // 🎭 跟配對當事人開聊→收掉配對（清泡泡+解凍）免凍結卡死；跟旁人聊不打斷等待中的小劇場
        S.talkTarget = npc;
    ...
    function _openLobbySettings() {
        _closeLobbySettings(); _closeDressRoom(); _closeNpcHistory(); _closeTheaterWin();
    ...
    function tryMount() {
        ...
        root.querySelector('.lstage-theater-btn').addEventListener('click', () => _openTheaterWin(S.theater && !S.theater.playing ? 'live' : 'review'));
