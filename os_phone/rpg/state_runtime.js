...
5. 校對：有沒有漏掉出場角色、有沒有編造、數值有沒有超出 desc 範圍、是否用點記法、沒變的有沒有誤寫進去
...
⛔【同時：拿這份目錄幫 memories 去重】你上面抽的 "memories"，請逐條對照下面目錄——若某條要記的事**已經在目錄裡了**（就算換句話說、簡繁體不同、措辭略異，只要是同一件事／同一狀態就算重複），就**從 memories 裡刪掉、不要重複記**；只保留目錄裡確實沒有的「新發生」的事。(玩家常重 roll／刪除重生同一段，重複記會讓記憶爆量、召回變糊。)

【全記憶帶碼目錄】
${catText}

→ 最終 JSON 需含 "recall_next" 欄位（你挑出的代號字串組成的陣列，數量看實際需要、沒有就 []），且 "memories" 已剔除與上面目錄重複的條目`;
    }
