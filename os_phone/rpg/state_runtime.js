...
請輸出嚴格 JSON（不包 markdown）：{ "memories": [ { "type":"...", "summary":"...", "text":"...", "tags":[...] } ] }`;
    ...
    //
    // 🗑️ 2026-08-01 移除「近期濾鏡」（原本只列近 3 章 + 這輪出現過的角色）：
    //    角色只要離場超過 3 章，頭像生成詞就從參考清單消失 → 副模型畫插圖時沒有錨點可對 →
    //    同一個角色每次回歸都被重畫成另一張臉。而「久違的角色回歸」正是最需要錨點的時候，
    //    濾鏡剛好把它濾掉，方向是反的。
    //    現在：當前世界所有「有頭像」的角色一律列出（avatar_cache 本身就只存畫過的人、且只看當前世界，
    //    天然有界）。代價是副模型 prompt 變長，覺得太肥再加「取最近 N 個」的數量上限，而不是時間窗。
    async function _buildLooksRef(state) {
        ...
        const avs = _charLooksRef(state, seen);   // 補漏：沒頭像但 AVS 有外觀的角色（本來就全列）
