...
${numberedText}`;
    ...
    async function deepConsolidate() {
        ...
        const prompt = [
            ...
            '5. 帳本清算（金錢/庫存/數量類欄位，重點任務）：掃【劇情總結】與【最近劇情】裡明寫的金額、收支、盤點句（「還剩X」「花了Y」「得到Z枚」），逐項核對這些欄位；對不上→以劇情明寫為準修正，每筆修正寫進 fixed 並附依據（例：「金錢 320→180，總結記載買馬花140」）。兩處都沒提到的數值保持原樣，嚴禁憑感覺改。',
