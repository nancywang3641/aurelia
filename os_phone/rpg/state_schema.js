...
- 每個角色的屬性分三層，順序固定：
  ...
  4.【當前盤算｜每個角色都必含、平鋪放角色名底下】：「當前目標」(string，這角色眼下在謀劃／想做的事，貼其人設動機；沒有填"無")、「時限」(string，期限或觸發條件，如"宴會結束前"/"三天內"，沒有填"無")。讓 AI 記得每個 NPC 各自在忙什麼、會主動推進，不只被動反應主角。
- desc 裡**必須寫明完整基礎組（固定錨點＋題材通用各是哪幾個）**，跑團時新角色登場才知道要套同一組。
- 正確範例（前五個固定錨點每角色完全一致，HP 是題材通用，最後一個才是各自特有；角色名與外觀值依實際世界填，這裡用佔位）：
  {
    "角色狀態": {
      "type": "object",
      "desc": "每個角色的即時狀態。固定錨點(每人都有、順序一致、平鋪)：髮色、眼色、體型、身分、好感度(0-100對主角)；題材通用：HP(0-100)；當前盤算：當前目標、時限。之後可依人設加專屬屬性。新角色登場自動套這組。主角MC好感度填 null。",
      "init": {
        "<角色A名>": { "髮色": "<依設定填>", "眼色": "<依設定填>", "體型": "<依設定填>", "身分": "<依設定填>", "好感度": 0, "HP": 100, "<特有屬性>": 80, "當前目標": "<這角色眼下想做的事>", "時限": "<期限或 無>" },
        "<角色B名>": { "髮色": "<依設定填>", "眼色": "<依設定填>", "體型": "<依設定填>", "身分": "<依設定填>", "好感度": 0, "HP": 100, "<特有屬性>": 50, "當前目標": "<這角色眼下想做的事>", "時限": "<期限或 無>" }
      }
    }
  }
