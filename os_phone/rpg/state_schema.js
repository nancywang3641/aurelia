...
- 每個角色的屬性分三層，順序固定：
  ...
  3.【角色特有】最後依該角色人設加 1~3 個專屬屬性（例：復仇者加「復仇執念」、騎士加「忠誠度」）。
- desc 裡**必須寫明完整基礎組（固定錨點＋題材通用各是哪幾個）**，跑團時新角色登場才知道要套同一組。
- 正確範例（前五個固定錨點每角色完全一致，HP 是題材通用，最後一個才是各自特有；角色名與外觀值依實際世界填，這裡用佔位）：
  {
    "角色狀態": {
      "type": "object",
      "desc": "每個角色的即時狀態。固定錨點(每人都有、順序一致、平鋪)：髮色、眼色、體型、身分、好感度(0-100對主角)；題材通用：HP(0-100)。之後可依人設加專屬屬性。新角色登場自動套這組。主角MC好感度填 null。",
      "init": {
        "<角色A名>": { "髮色": "<依設定填>", "眼色": "<依設定填>", "體型": "<依設定填>", "身分": "<依設定填>", "好感度": 0, "HP": 100, "<特有屬性>": 80 },
        "<角色B名>": { "髮色": "<依設定填>", "眼色": "<依設定填>", "體型": "<依設定填>", "身分": "<依設定填>", "好感度": 0, "HP": 100, "<特有屬性>": 50 }
      }
    }
  }
...
- **必含一個扁平欄位「主線目標」（type string）**：整個故事的核心目標／最初接下的任務，一句話講清「為什麼出發、最終要達成什麼」。這是**穩定錨點**——開場確立後固定不動、不隨劇情每輪變（跑團越深防 AI 忘記初衷）。它與「當前任務」不同：當前任務是眼前這一步、每輪會變；主線目標是整條主線、幾乎不動。init 直接依世界設定／開場填入真正的主線，不要留空泛佔位。
