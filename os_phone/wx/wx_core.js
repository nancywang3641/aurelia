    ...
    win.wxApp = {
        ...
        storyTidyAi: async function() {
            ...
            const sys = '你是資料整理工具。下面 JSON 是從跑團劇情解析出的手機聊天室。寫劇情的模型同時要寫正文和很多格式，聊天室資料常寫錯，你負責找出錯誤。\n'
                + 'me＝主角（這支手機的主人）的各種叫法；contacts＝通訊錄裡已有的人名寫法；rooms＝每間聊天室的 id（程式內部用的代號）、name（畫面上顯示的聊天室名）、members（名單）、speakers（實際發話的人）、count（訊息數）、sample（最近幾句對話）。\n'
                + '要找三種錯：\n'
                ...
                // 🚨 這句以前是「只要不確定就不要列」，跟上面「群聊一定要給一個名字」正面打架 ——
                //    模型照這句跳過不取名是合理的。要限定它只管哪幾項（別在協議裡留互相矛盾的條文）。
                + '第一項與第三項只要不確定就不要列，寧可不合併、不統一，那兩個清單可以是空的。\n'
                + '第二項不一樣：名字是代號的那幾間一定要給新名字，不可以因為不確定就跳過。\n'
