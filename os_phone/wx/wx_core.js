...
(async function () {
    ...
    // 皮決定顏色，那顆決定明暗，兩個各管各的。
    const WX_THEMES = [
        { id: '',       name: '經典綠' },     // 空字串＝不掛任何皮的 class，用預設那組
        //  🚨 皮的名字只講長相、不講是哪個 App：這支聊天 app 其實是混的（有些做法來自 LINE），
        //     掛真實 App 的名字不老實，她之後也打算把整支重新命名。
        { id: 'cream',  name: '奶油' },
        { id: 'indigo', name: '靛藍' },
    ];
    ...
    win.wxApp = {
        ...
        storyTidyAi: async function() {
            ...
            // 📋 這份指令是 Rae 2026-09-20 自己重寫的，逐字照她給的那份，不要自己改寫、
            //    也不要「順手補一條」—— 上一版我自己加的那句「看不出群名就把成員名字串起來」
            //    直接把她的群變成一串人名。要動先問她。
            // 🚨 欄位名要跟下面 payload 給的對得上（me／contacts／rooms 的 id・name・members・speakers・count・sample）。
            const sys = [
                '你是「聊天室資料修復器」。',
                '輸入是一份 JSON，包含：',
                '',
                ...
                '你的工作只有三件事：',
                '1. 合併重複聊天室',
                '找出「實際上是同一間聊天室，卻用了不同 id」的資料。',
                '只有在以下資訊明顯一致時才合併：',
                '',
                '* 聊天室名稱相同或明顯同義',
                '* 成員相同或高度重疊',
                '* `sample` 顯示是同一段對話或同一事件的延續',
                '',
                '這三條要同時成立才算同一間。只中一兩條就不要列。',
                '成員名單的比法：扣掉 me 裡那些名字之後，兩邊剩下的人要是同一批。'
                  + '有一邊多出或少掉一個人以上，就當成不同的聊天室，即使對話看起來像在講同一件事。',
                '',
                '不要只因為名稱相似就合併。',
                '確認為同一聊天室後：',
                '',
                '* `canonicalId` 使用該組 `count` 最大的聊天室 id',
                '* `ids` 列出這組全部 id',
                '',
                '`id` 是程式識別碼，除「確認重複聊天室」之外，不得修改或猜測。',
                '2. 修正聊天室顯示名稱',
                '只處理 `name` 明顯不適合作為畫面顯示名稱的聊天室。',
                '以下情況必須改名：',
                '',
                '* `name` 與 `id` 完全相同',
                '* `name` 明顯是程式代號、流水號或識別碼',
                '* `name` 只是把成員姓名用逗號、頓號等串起來',
                '',
                '命名方式：',
                '私聊',
                '扣除 `me` 後只剩一位成員時，使用該人物的標準姓名。',
                '群聊',
                '根據 `members` 與 `sample` 判斷這個群聊的用途，取一個自然的中文短名稱。',
                '要求：',
                '',
                '* 最多 8 個中文字',
                '* 不使用英文、編號或 id',
                ...
                '標準名稱優先順序：',
                '',
                '1. `me` 中已有的寫法。me 裡列的全都是同一個人（手機主人）的不同叫法，'
                  + '要挑一個當標準時，挑裡面字最完整的那個。',
                '2. `contacts` 中已有的寫法',
                '3. 否則使用自然的繁體中文姓名',
                '',
                '只有高度確定是同一個人才列入。',
                '如果只是名字相似、可能是不同人物，不要合併。',
                '判斷原則',
                '優先避免錯誤合併。',
                '',
                '* `merge`：不確定就不要列',
                '* `people`：不確定就不要列',
                '* `rename`：如果名稱明顯是代號，必須修正',
                '',
                '不要新增不存在的人物。',
                '不要改寫劇情。',
                '不要整理訊息內容。',
                '不要解釋判斷理由。',
                '只輸出合法 JSON：',
                '{"merge":[{"canonicalId":"room_id","ids":["room_id_a","room_id_b"]}],"rename":[{"id":"room_id","name":"顯示名稱"}],"people":[{"name":"標準姓名","variants":["其他寫法"]}]}',
                '沒有資料時使用空陣列。',
                '禁止輸出 Markdown、註解或 JSON 以外的文字。',
        ...
        // 🎙 點語音泡泡：她自己錄的就播真的聲音、字幕跟著打開，播放中再點一下停；
        //    沒有錄音的（AI 傳的、舊的打字語音）照舊點開看字
        toggleVoice: async function(el, txt) {
            const box = el.querySelector('.wx-vmsg-trans');
            const t = decodeURIComponent(txt || '');
            const row = el.closest('.wx-msg-row');
            const idx = (row && row.dataset.msgIdx != null) ? parseInt(row.dataset.msgIdx, 10) : -1;
            const chat = GLOBAL_ACTIVE_ID ? GLOBAL_CHATS[GLOBAL_ACTIVE_ID] : null;
            const msg = (chat && chat.messages && idx >= 0) ? chat.messages[idx] : null;
            if (msg && msg.voiceAudio) {
                ...
                try {
                    ...
                    _voicePlaying = { el: el, audio: audio, url: url };
                    el.classList.add('is-playing');
                    audio.onended = () => this._stopVoicePlay();
                    audio.onerror = () => { this._stopVoicePlay(); AUI.toast('這段錄音播不出來'); };
                    await audio.play();
                } catch (e) {
                    console.warn('[WX] 播放語音失敗:', e);
                    this._stopVoicePlay();
                }
                return;
            }
        ...
        voiceDownload: function () { this._vsPrepare(true); },
        _vsPrepare: async function (fresh) {
            const VI = win.OS_VOICE_INPUT;
            const el = this._vsEl();
            if (!el || !VI) return;
            this._vsState(fresh ? 'downloading' : 'preparing');
            const note = el.querySelector('#wxVoiceNote');
            const bar = el.querySelector('#wxVoiceBar');
            if (fresh && note) note.textContent = '下載中 0%';
            try {
                await VI.prepare((p) => {
                    if (el.hidden) return;
                    if (p.stage === 'download' && el.dataset.state === 'downloading') {
                        if (bar) bar.value = p.percent;
                        if (note) note.textContent = '下載中 ' + p.percent + '%';
        ...
        // --- 發送消息 (🔥 關鍵修復點) ---
        // extra：要一起存在這則訊息上的欄位（語音的 voiceAudio／voiceSec），不進正文
        sendMsg: async function(el, contentOverride = null, extra = null) {
