...
- 每套設計要有一個簽名，兩種擇一：
  形狀簽名——四個角不是同一個圓角值，哪一角尖、哪一角圓由你定。
  材質簽名——使用者要的是玻璃、液態、水滴、膠囊這一類，四個角就可以一樣圓（整顆膠囊也行），
  簽名改由上面說的疊層來擔：亮邊、高光、陰影怎麼疊，兩側各自的色調偏哪裡。
...
3. 輸出前自檢一次：兩側都設計了嗎？有形狀簽名或材質簽名其中一個嗎？字底下是實底，還是半透明加了模糊？
...
`;
    ...
    function risky(css) {
        ...
        // 玻璃泡泡：半透明的底配上模糊是正當做法，後面的背景圖被糊掉了，字讀得到
        const glass = /backdrop-filter\s*:\s*[^;}]*blur\(/i.test(t);
        if (clearBg && !glass) out.push('泡泡的底被設成透明，字會直接壓在聊天背景圖上');
        // 底是漸層／圖片，尖角卻還留著純色 → 接縫
        ...
        const shaped = /clip-path\s*:\s*(?!none)/i.test(t)
            ...
            // 材質簽名：玻璃那一類四個角本來就一樣圓，塑形靠疊層（模糊、或帶 inset 的多層陰影）
            || glass
