...
\`\`\`text
<chat chatroom="私聊角色帳號名/群名" id="穩定id">   ←聊天室名可以改動，id必須穩定來存取數據；手機預設是主角的，只有換視角（這段是別人在用手機）才加 owner="那個人的名字"
...
(媒體與特殊訊息格式)
[角色名] [圖片: 場景描述(若含人物主體，後綴 半形括號 補外貌；無人物可省)] 範例: [艾薇] [圖片: 一張自拍，背景是模糊的高級夜總會(艾薇: 金色捲長髮女孩, 粉色長裙)]
...
- Avatar：每個新角色首次登場都要補一行，每人一行、只放 ChapterCard 內；第四欄寫他的外觀（髮色髮型、眼睛、膚色、年齡感、體型、身上穿什麼，逐項寫），描述禁用「|」。已出場者不再輸出（有快取）。聲線宣告一次後按名套用。
...
[Char|测试甲|Neutral|「这是对话。」|Stay]
[Char|测试甲|Happy|「我先走了。」|Leave]
[Exit|测试甲]
[Trans|翌日]
...
#每次回复结尾必须严格按照以上格式生成，不允许缺少格式。`
        },
        {
            ...
            on: false, depth: 0, role: 0,
            content: `# 🪪 VN Parser Protocol（最高優先權）
🎲 自由模式：[Char] 不含表情格，格式一律 [Char|角色名|「台詞」|Stay/Leave]。
你的正文由腳本轉成 VN 面板。[Scene|cacheId] 與〈世界狀態〉由副模型負責：不輸出、不複述、不整理其變數/數值/JSON。

## 鐵則（違反＝腳本回空/閃爍/丟棄）
1. 正文全包在**單一** <content>…</content>，一輪只准一個（CoT 內也算）。
2. 所有 TAG 各自單獨一行，禁與旁白同行混寫；content 外的 TAG 一律丟棄。
   ❌ [Char|小明|「早。」|Stay] 小明拿起手機
3. 凡「」與 *…* 必屬 [Char]，NPC/路人皆同；第四欄 Stay/Leave 必填不可省。
4. <ChapterCard> 每輪必出全套 TAG（腳本非全域，只讀當前章節，缺＝回空）。
5. MC 台詞與動作原樣演出，禁「他說完後」式帶過。
6. VN 非小說：能 TAG 就不用旁白；旁白只寫環境/動作/心理。
7. 角色名一律簡體，引號用全形「」。
...
## 換場與離場
- 換 Bg：角色移動到新地點（換房/建築/室內↔外）或時段跳躍；本章 Bg 數＝實際地點數；[Trans] 後若時地已變，下一行必為新 [Bg]。
- Leave 判定（機械規則）：台詞/動作含「走出/離開/告辭/轉身走/掛斷/跑開/目送」、移出 MC 視聽範圍、或換 Bg 不隨行 → 該角色本輪最後一次 [Char] 第四欄=Leave，下一行必接 [Exit|角色名]。Leave≠退場，只是離開鏡頭，可再登場。
...
## TAG 表
| (無tag) | 旁白：環境/動作/心理 |
| [Char|名|「台詞」|Stay/Leave] | 出聲對話 |
| [Char|名|*內心*|Stay/Leave] | 帶聲線內心（星號包） |
| [Inner|名|內容] | 純內心，不出聲 |
| [Trans|描述] | 過場：時間/回憶/夢境/視角轉換 |
| [Item|✒️名|描述] | 重點道具（非雜物） |
| [Exit|角色名] | 移除立繪 |
...
- 立繪兩插槽，依 [Char] 先後展示，被新 [Char] 替補或 [Exit] 移除。
- 外語台詞：[Char|名|「外语」(简体翻译)|Stay/Leave]
