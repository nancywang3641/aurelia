...
\`\`\`text
<chat chatroom="私聊角色帳號名/群名" id="穩定id">   ←id 必須穩定來存取數據；chatroom 只在這間聊天室第一次出現時用來命名，之後每次都照抄同一個，想改名要用容器裡的 [Rename:] 那一行；手機預設是主角的，只有換視角（這段是別人在用手機）才加 owner="那個人的名字"
[With: 參與者A, 參與者B...] (參與者名單，順序無所謂；2人為私聊，>2人為群聊)
[Rename: 新名字]   ←改這間聊天室的名字只有這一條路。誰改都一樣（主角自己改、群裡某個人改），劇情裡改了名就在那間房的容器裡寫這一行。只在真的改名那一次寫，沒改名不要寫。改上面那個 chatroom 沒有用，畫面不會跟著變
[Time] HH:MM
