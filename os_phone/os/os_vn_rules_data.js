...
[You] 發送的訊息
[角色名] 發送的訊息
[角色名] [recall] 被撤回的那句話   ←傳了又收回；要寫出原本那句，手機上會先出現一下再變成撤回提示。主角撤回自己的也這樣寫
[系統] 系統提示訊息
[系統] 你已刪除好友「角色名」   ←主角刪掉這個人的微信時寫，刪了之後又加回來寫 [系統] 你已添加了角色名；主角把這個人拉黑寫 [系統] 你已將「角色名」加入黑名單（聊天室還在，那個人傳不進來），放出來寫 [系統] 你已將「角色名」移出黑名單
[系統] 角色名開啟了朋友驗證，你還不是他（她）朋友   ←反過來，某個角色把主角刪了時寫；某個角色把主角拉黑時寫 [系統] 消息已發出，但被對方拒收了。主角之後在那間聊天室送出的訊息對方都收不到（程式會自己補上這一句），直到那個角色重新加主角好友（寫 [系統] 「角色名」请求添加你为朋友）或主角被加回來
[Kick: 被移出的人|動手的人]  [Leave: 退群的人]  [Join: 加入的人|邀請的人]  [Mute: 被禁言的人或all|動手的人]  [Unmute: 同上]   ←標籤名照抄英文，動手的人可省略；被移出、退群、被禁言的人之後不在群裡說話
上面刪好友、拉黑、朋友驗證、拒收那幾行只寫在私聊；Kick／Leave／Join／Mute 只寫在群聊
...
<moments>
[Post|編號|發文的人|動態內容]
[Photo|編號|給人看的一句中文 >> 畫圖用的英文句子]   ←這則動態附的照片，一張一行，寫在那則 [Post] 下面，編號跟那則 [Post] 一樣。兩段中間一定要有 >>，寫法跟聊天室的 [圖片:] 一樣
[Like|編號|按讚的人]   ←一個人一行
...
- 開場 <ChapterCard> 內、[Bg] 行後，每角色一行；名字與 [Char] 同步。
- 最後兩格分開寫：前一格是長相（這個人不會變的樣子），後一格是這張頭像的穿著、表情、鏡頭、背景。插圖只拿長相那格，衣服背景交給當下劇情；寫錯格，每張插圖都會帶著這身衣服和這個背景。除了這兩格之間，內文禁用「|」。
...
- handsome/beautiful 要用就得同時有錨點。非人類沒有人類老化外觀也要選一檔，錨點改寫在體表（甲殼磨損、鱗片褪色、羽毛斑白）。`
        },
        {
            ...
            on: false, depth: 0, role: 0,
            content: `## 🪪 [AVATAR] 新角色首次登場輸出一行，已出場過的不再輸出
[Avatar|名|聲線|1girl/1boy, {{種族標籤組}}, {年齡感}, looking at viewer, bust shot, {體型}, {膚色體表}, {瞳色}, {眼型}, {髮長}, {髮型}, {瀏海}, {標誌特徵}|{1-2件服裝}, {表情}, {背景} background]
- 開場 <ChapterCard> 內、[Bg] 行後，每角色一行；名字與 [Char] 同步。
- 最後兩格分開寫：前一格是長相（這個人不會變的樣子），後一格是這張頭像的服裝、表情、背景。插圖只拿長相那格，衣服背景交給當下劇情；寫錯格，每張插圖都會帶著這身衣服和這個背景。除了這兩格之間，內文禁用「|」。
- Danbooru 標籤逗號分隔，禁長句。開頭必須 1girl／1boy（人數標籤，非人類照寫）。
- 單人胸像。禁環境／一次性／群體角色、完整場景、群眾、戰鬥、腰部以下（全身腿鞋蹄尾）。負面詞與 solo/portrait/bust shot 腳本會加。
- {{}}＝加重。只加種族標籤組與年齡錨點；種族年齡寫在長相那格。
...
- 非人類沒有人類老化外觀也要選一檔，錨點改寫在體表（甲殼磨損、鱗片褪色、羽毛斑白）。`
        },
        {
            ...
            on: true, depth: 0, role: 0,
            content: `## 🪪 [AVATAR] 新角色首次登場輸出一行，已出場過的不再輸出
[Avatar|名|聲線|A {adult/teen/elderly+主詞} with {髮色髮型}, {瞳色} eyes, {體型}, {種族標籤}, {標誌特徵}, {種族特徵}|wearing {上衣}, {下褲}, {裝飾物}, {表情}, {背景} background]
- 開場 <ChapterCard> 內、[Bg] 行後，每角色一行；名字與 [Char] 同步。
- 最後兩格分開寫：前一格是長相（這個人不會變的樣子），後一格是這張頭像的穿著、表情、背景。插圖只拿長相那格，衣服背景交給當下劇情；寫錯格，每張插圖都會帶著這身衣服和這個背景。除了這兩格之間，內文禁用「|」。
- 單人胸像。禁環境／一次性／群體角色、完整場景、群眾、戰鬥、腰部以下（腿鞋蹄尾）。負面詞腳本會加。
- ()＝加重。只包種族特徵與 40+ 錨點。
- 體型必填，兩個詞：身高（short／average height／tall）＋體格（slim／lean／athletic／muscular／stocky／heavyset／curvy 挑最貼的一個）。寫這個人固定的身材，不隨場合變；寫在長相那格，插圖每一張都照這個身材畫，不寫的話每張圖的胖瘦壯會各畫各的。
...
- 非人類沒有人類老化外觀也要選一檔，錨點改寫在體表（甲殼磨損、鱗片褪色、羽毛斑白）。`
        },
        {
            ...
            on: false, depth: 0, role: 0,
            content: `## 🪪 [AVATAR] 新角色首次登場輸出一行，已出場過的不再輸出
[Avatar|名|聲線|A {adult/teen/elderly+主詞} with {髮色髮型}, {瞳色} eyes, {種族標籤}, {標誌特徵}, {種族特徵}|wearing {上衣}, {下褲}, {裝飾物}, {表情}, {背景} background]
- 開場 <ChapterCard> 內、[Bg] 行後，每角色一行；名字與 [Char] 同步。
- 最後兩格分開寫：前一格是長相（這個人不會變的樣子），後一格是這張頭像的穿著、表情、背景。插圖只拿長相那格，衣服背景交給當下劇情；寫錯格，每張插圖都會帶著這身衣服和這個背景。除了這兩格之間，內文禁用「|」。
