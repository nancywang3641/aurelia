...
任務：為這些變數設計「條件規則」——當變數達到某門檻時，角色的「關係」與相處方式如何隨之改變。先規劃再寫，別給又短又亂套的條目。

【好感度規則（最重要；只要追蹤的變數裡有「好感度」就一定要做）】
好感度通常在「角色狀態」這類 object 容器內、每個角色各一份；path 用點記法：容器名.角色名.好感度（例：角色狀態.卡蜜拉.好感度），角色名填真實角色名。
- 對劇情中每個「重要角色」（主要登場、與主角有關係線的角色，通常 2～6 人；一次性路人不算），各生成「正好 3 條」，沿其好感度由低到高分三檔。
- ★★ 好感度＝「關係階段」一階一階往前推，不是情緒強度。每一條都要「先點明這門檻對應哪個關係階段」，再寫該階段對主角的相處方式：
   · 低檔（例 <=30）：關係生疏／戒備／公事公辦 → 寫這個距離感下怎麼對待主角。
   · 中檔（例 50～70）：關係轉為熟識／朋友／開始在意 → 寫拉近後的相處。
   · 高檔（最後一條，例 >=85）：關係「質變」進更近一階（曖昧→戀人那種）→ 直接用「那個新關係本身」的相處模式：稱呼、界線、肢體分寸、把主角放第幾位、佔有與否，全部隨新關係改寫。
- ⛔ 嚴禁把「關係沒變、只是同一種關係的情緒變更激進／更黏／更兇」當成高好感——那是情緒波動，不是好感度。高好感必須是「關係更近一階」，行為跟著新關係走，不是把原本態度放大。
- 主角MC／{{user}} 本人不生規則（其好感度為 null）。

【其他變數（選配）】
若有其他有意義的變數（HP、信任度、理智值…），可另沿其高低各給 2～4 條漸進規則：同一變數一條由淺入深的線、彼此不矛盾、貼著世界觀最自然的走向。

【行為說明寫法】要「能直接演出來」：先寫關係定位，再寫具體神態、語氣、肢體分寸、對待主角的方式；禁止只丟空泛形容詞標籤。

【輸出格式】每行一條，嚴格 5 欄、以 | 分隔；除了規則行不要任何多餘文字（不要標題、不要說明、不要回述範例）：
名稱 | 變數路徑 | 運算子 | 比較值 | 行為說明
- 變數路徑：一般變數照原樣填；object 容器內屬性用點記法（容器名.角色名.屬性，如 角色狀態.卡蜜拉.好感度）。運算子只能用 >= <= > < = !=
- 名稱與行為說明都用繁體中文純文字，禁止任何 Markdown 或裝飾符號
- 行為說明約 40～90 字、具體可演，但務必單行、不可換行、其中不可出現 | 符號
- 每個重要角色的好感度正好 3 條；其他變數各 2～4 條`;
