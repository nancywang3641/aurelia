...
- tags: 檢索用關鍵詞陣列。以「能定位這條記憶」的詞為主——事件詞（發生什麼／做了什麼）、地點、物品、概念或狀態變化。
    ⚠️ 主角名人人皆知，當 tag 形同沒標；除非某角色是這條的「新登場／關係生變」主體，否則別把人名（尤其主角名）塞進 tags，更別整條 tags 只有一串人名。
    例外：type 為 "dialogue" 時，tags 才放「說話的角色名」（語氣索引靠它定位）。
