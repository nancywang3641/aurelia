...
- text:【細節用】較完整的描述（1～3句），保留之後可能要回想的具體細節。若 type 為 "dialogue"，請寫「<角色名>的說話風格／口癖 ＋ 一句最能代表其性格的原句台詞」，盡量保留原話、別改寫成轉述。若 type 為 "sex"，只寫這一次發生了什麼：在什麼情況下、誰主動、兩人當下的狀態，別寫露骨過程細節；不要寫兩人是什麼關係、關係變成怎樣——關係會隨劇情改變，那是 type:"relationship" 的事。
...
- weight: 0～1 的數字，這條在整個故事裡的份量。判準是「之後還會不會被牽動」：改變了關係或處境、留下還沒收的線、代價很大的選擇 → 高；當下就結束、之後不影響任何事 → 低。
    ⚠️ 判的是「之後會不會回頭影響」，不是「當下有多激烈」。拿不準就給 0.5。

動手寫之前，先把這一章照下面四項從頭到尾過一遍（只是你自己檢查用，不用寫出來）。
這四類的共同點：當下看起來常常只是順手的小動作，可是一旦沒記到，幾章之後的劇情就會寫出跟它矛盾的內容，而且沒有人發現得了。
1. 東西的去向：這一章裡每一件被拿起、交給別人、藏起來、丟掉、弄壞、用掉、花掉的東西，章節結束時在哪裡、在誰手上、還在不在。
   「已經不在了」的東西一定要記一條 type:"item"，text 寫明是誰、怎麼弄沒的、最後去了哪裡——角色當時覺得它不重要也照記。
   之後的劇情最容易寫錯的就是「那個東西其實早就不在他手上了」。
2. 誰知道什麼：有沒有哪件事只有一部分人知道。記一條 type:"event"，text 寫明誰知道、誰不知道、誰以為是怎樣（以為的內容跟事實不一樣時，兩個都寫）。
   這種條目還沒有被說破之前都算還懸著，weight 給 0.7 以上。
3. 說出口的話：答應、拒絕、謊話、威脅、開出的條件（有金額、期限就照實寫）。
4. 回不去的事：受傷、死亡、決裂、身分或祕密曝光、地方被毀。
上面四類只要發生了就要有一條，不受 weight 高低影響——weight 低只是之後比較晚被想起來，沒記就是永遠不存在。
...
- ⭐ type:"sex"：主角跟某 NPC 發生性事就記一條（這次發生了什麼，不寫關係），防 NPC 後續見面忘記有過性事、變成拔屌無情 OOC。兩人的關係或感情因此有了變化，另外記一條 type:"relationship"
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                await win.OS_DB.saveVnMemory({
                    ...
                    text: entry.text || '',
                    tags: entry.tags || [],
                    // 這條在故事裡的份量（副模型判「之後還會不會被牽動」）。召回時留幾格給
                    // 份量重、但跟當下劇情不相似所以撈不到的事——沒有它，二十輪前埋的線在
                    // 話題轉開之後就再也浮不上來。副模型沒吐或吐了怪東西一律當 0.5（中性），
                    // 舊記憶沒這欄也是同樣待遇，所以不必回填。
                    weight: (function () {
