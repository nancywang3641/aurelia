            ...
            try {
                ...
                if (multiObj.length) {
                    fullPrompt +=
                        ...
                        // 🚨頭像是人像。做成寬>高的橫框，臉會被 object-fit:cover 切成一條，怎麼看都像壞掉。
                        `★【頭像框鐵則】{{@avatar}} 是人物頭像：外框一律正方形或直式（高不小於寬），` +
                        `例如 aspect-ratio:1/1 或 3/4，配 object-fit:cover; object-position:top（優先保住臉）。\n` +
                        `　 嚴禁做成橫版長方形（寬大於高）那種寬扁的框——臉會被切成一條。` +
                        `圓形、圓角方形、切角方形、加描邊都可以，就是不准壓扁。也不要用 width/height 各自寫死造成變形。\n` +
