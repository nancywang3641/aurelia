    ...
    function bindPackEditorEvents(container) {
        ...
        if (btnPreset) {
            btnPreset.onclick = async () => {
                if (!confirm('套用「簡易預設」變數包？\n通用四組狀態：\n• 角色狀態（形象/身分/好感度/心情/所在）\n• 主角狀態（體力/心情/持有）\n• 場景（時間/地點/氛圍）\n• 進行中目標\n免讀世界書、一鍵套；想更貼世界觀可改走「AI 從世界生成」。')) return;
                ...
                try {
                    ...
                    const pack = {
                        id: 'pack_' + Date.now(),
                        name: '簡易預設（角色/主角/場景/目標）',
                        notes: '輕量通用預設（免讀世界書、一鍵套）。追蹤四組：角色狀態(形象/身分/好感度/心情/所在)、主角狀態(體力/心情/持有)、場景(時間/地點/氛圍)、進行中目標。全部走點記法增量更新、不整包重寫。想更貼世界觀就改走「AI 從世界生成」。',
                        // 物件型一律給「縮排結構範本」(parseTree 解析)，副模型才看得到固定子欄位、不會每輪亂編結構而導致覆蓋。
                        // 多實體容器(角色狀態/進行中目標)外層用變數名包裝當範本(會被剝掉)；單一實體(主角狀態/場景)直接列欄位。
                        variables: [
                            {
                                name: '角色狀態',
                                type: 'object',
                                defaultValue: '角色狀態:\n  形象:\n    髮色: 待定\n    眼色: 待定\n    體型: 待定\n  身分: 待定\n  好感度: 0\n  心情: 平靜\n  所在: 待定',
                                desc: '每個「有意義的具名角色」的即時狀態。固定基礎屬性(每個角色同一組、順序一致)：形象(髮色/眼色/體型)、身分、好感度(0-100對MC)、心情、所在位置。'
                                    + '【更新方式】用點記法只更新有變化的欄位(例：角色狀態.愛麗絲.好感度)，不要整包重寫、沒變的別動，避免洗掉其他角色與屬性。'
                                    + '【誰要記】只記有名字、對劇情有份量的人物角色；雜魚/怪獸/野生動物/路人敵兵這類不要當角色(除非是有劇情份量的具名角色)。'
                                    + '【主角例外】主角 / MC / {{user}} 本人由「主角狀態」變數單獨追蹤，這裡不要重複記主角。'
                            ...
                            },
                            {
                                ...
                                type: 'object',
                                defaultValue: '體力: 100\n心情: 平靜\n持有: 待定',
                                desc: '主角 / MC / {{user}} 本人的即時狀態。固定欄位：體力(0-100)、心情、持有(身上關鍵物品或金錢，簡述)。用點記法只更新有變化的(例：主角狀態.體力)。'
                            },
                            {
                                name: '場景',
                                type: 'object',
                                defaultValue: '時間: 待定\n地點: 待定\n氛圍: 待定',
                                desc: '當前場景。固定欄位：時間(劇情當下時間/時段)、地點(目前所在)、氛圍(整體氣氛一詞)。場景變了才更新，用點記法(例：場景.地點)。'
                            },
                            {
                                name: '進行中目標',
                                type: 'object',
                                defaultValue: '進行中目標:\n  描述: 待定\n  狀態: 進行中',
                                desc: '正在進行、尚未完成的任務/目標(已完成的移除)。每個目標固定欄位：描述、狀態(進行中/已完成)。key 用目標簡名。用點記法(例：進行中目標.尋找線索.狀態)。沒有就留空。'
                            }
                        ],
