    ...
    function bindFurnaceEvents(container) {
        ...
        container.querySelector('#furnace-start-btn').onclick = async () => {
            ...
            try {
                ...
                fullPrompt +=
                    `\n\n【🚫 防無聊鐵則｜最重要】你最大的老毛病＝把資料塞進「卡片裡再套卡片」、每次只換顏色和名稱。嚴禁：\n` +
                    ...
                    `【📏 高度感】別讓面板「內容多高就多高」塌成一條：根容器給合理 min-height；有結構的用 display:flex;flex-direction:column；長列表用 max-height + overflow:auto 內捲；維持刻意穩定的版面比例（仍須符合上方手機相容鐵則）。\n\n` +
