...
(function () {
    ...
    const _PANEL_RULES =
        ...
        '【按鈕長什麼樣】從這個世界自己的東西長出來。先問:這裡的人用什麼材料做東西、' +
        '他們怎麼標示與指引一件事、要按下或開啟什麼的時候手會碰到什麼、他們的字與記號寫在什麼上面。' +
        '按鈕就照那個做——它應該像是這個世界裡真的存在的物件或標示,而不是一個貼了世界觀貼紙的通用元件。\n' +
        // 🚨要一致的是「同一個世界內」,要不同的是「世界與世界之間」。
        //   實測問題出在後者:每個世界的按鈕都長成同一種「圖示＋文字＋細框的清單列」,
        //   換了美術方向也沒換樣子——因為規則只講了功能,沒要求造型跟著美術方向走。
        '　同一個世界裡的八顆要看得出是同一套語言(主次可以有差別,但語言一致)。\n' +
        '　🚨但這套語言必須是上面那個美術方向長出來的東西,換一個世界就該是完全另一種造型。' +
        '不要每次都做成「小圖示＋文字＋細邊框」的直排清單——那是通用後台介面的樣子,不是任何一個世界的樣子。' +
        '這個世界的按鈕應該讓人一眼看出它屬於這裡,而不是套用在哪個世界都成立。\n' +
