...
(function () {
    ...
    async function _drawSeeds(hint) {
        const prompt =
            ...
            '[{"genre":"{題材類型,不超過8字,例如:劍與魔法/仙俠修真/神話史詩/海洋與深淵/蒸汽工業/民俗怪談}","name":"{世界名,不超過8字,有記憶點}","concept":"{一句話概念,不超過25字}","twist":"{一條違反常識的核心規則,不超過25字}","style":"{視覺風格,不超過10字}","lure":"{玩家前往的理由/可獲得什麼,不超過20字}","danger":"{主要危險,不超過15字}","crisis":"{世界目前正在發生的危機,不超過20字}"}]\n' +
            (hint
            ...
            'twist 是種子的靈魂,必須是「規則」不是「景觀」:這個世界某件基本事物的運作方式與現實相反、或掛鉤到意想不到的東西,而且滲透所有人謀生、交易、社交的日常——不是只是看起來奇特。4 顆種子的 twist 必須分別掛在不同領域(生存/經濟/身體/情感/語言/時間/記憶/身分等任選),並且用該題材自己的語言寫成(劍與魔法就寫成魔法、神諭、血脈、契約的法則;海洋題材就寫成潮汐、深淵、歌聲的法則)。\n' +
