...
(function () {
    ...
    async function _drawSeeds(hint) {
        const prompt =
            ...
            'twist 是種子的靈魂,必須是「規則」不是「景觀」:一條這個世界的人天天要應付、而且會左右他們怎麼謀生與怎麼相處的規矩,不是只是看起來奇特。\n' +
            '寫 twist 時不要用「某個抽象概念等於、變成或存放在某個具體東西」的句型——那是同一個套路換皮,四顆並排會像同一個模子印出來的。改成寫「這裡的人被什麼事情限制住、因此不得不怎麼做」,句子裡要看得見人的行為,不是只有一個奇觀設定。\n' +
            '4 顆種子的 twist 不可以是同一種句型,也不要全都圍著同一件事打轉。用該題材自己的語言寫。\n' +
            'daily 要寫這條規矩讓當地人每天實際在做的事,具體到看得見動作與場面,不要抽象形容。\n' +
