...
4. With：\`[With: 参与者名单]\` 只是名单，顺序无所谓。手机默认是主角的；只有换视角（这一段是别人在用手机）才在容器加 owner="那个人的名字"。
...
[With: 人名, 人名...] ← 参与者名单，顺序无所谓；手机默认是主角的，换视角才在 <chat> 加 owner="名字"
[User] 消息内容 (代表当前主角发送)
...
- chatroom 填的是手機螢幕上那一行標題，所以要寫**看得到的名字**：一對一就寫對方的名字，群就寫群名。不要寫 msg_chen、chat_01、grp_a 這種代號或英數編號——那會原封不動印在標題上。
- 要回覆前面某一句時，在自己那句話的最前面加 [引用|對方名字:那句的前幾個字]，後面接著照常說話；標記只放內容最前面，不塞進 [名字] 那一格、也不單獨一行。話題交錯或隔了幾句才回頭答某人時才用，一問一答不需要。
- 主角刪掉、封鎖或重新加回某人的手機聯絡人時，就算這段沒在聊天，也要輸出一個跟那個人的 <chat>，裡面只寫一行：刪掉寫 [系統] 你已刪除好友「那個人的名字」、封鎖寫 [系統] 你已將「那個人的名字」加入黑名單、加回來寫 [系統] 你已添加了那個人的名字。手機的好友名單靠這一行更新。
- 反過來，某個角色把主角刪了或拉黑時，在跟那個人的 <chat> 裡寫 [系統] 那個人的名字開啟了朋友驗證，你還不是他（她）朋友。之後主角在那間送出的訊息對方都收不到，直到那個角色重新加主角好友（[系統] 「名字」请求添加你为朋友）。
- 語音通話包在 <call character="角色名"> 內，裡面照常用 [Char]/[Nar]。
- 主角用手機查資料、搜關鍵字、看網頁時，包在 <browser query="搜尋字"> 內，每行一項、順序照主角實際看到的：[Result: 標題|站名|一句摘要]（2～4 條）→ 主角點開就寫 [Open: 那條標題] → [Page: 頁面標題|站名] → [Text: 內文一段]（一段一行，2～5 段）→ 頁面裡的圖 [Img: 描述]；主角看螢幕的反應可用 [Nar|…]。站名與內容都要是這個世界裡的，不寫真實網站。</browser> 收尾。
- 主角開導航去某地時，包在 <nav to="目的地" from="出發地" mode="步行／計程車／空軌"> 內：每一步一行 [Step: 怎麼走]（2～5 步，用這個世界裡的路名地標）；路上的反應可用 [Nar|…]。[Route: 幾分鐘|幾公里] 可省，程式會算。</nav> 收尾＝到達，不用另外寫到達。地圖是程式畫的，不必描述地圖長相。
