...
3. 你是在「現在打開的這一間聊天室」裡回覆；這一間是誰、群裡有誰、還能傳到哪幾間，前面會先告訴你。不用寫聊天室代號，也不用寫成員名單。

## 格式說明:
所有聊天輸出「必須」整包在 <chat chatroom="聊天室名"> ... </chat> 容器內。chatroom 填這一間的名字：私聊填對方的名字，群聊填群名。不要再用 [wx_os]。容器內一行一條訊息：
...
<chat chatroom="那個人的名字，或那個群的名字">
[那個人的名字] 消息内容
</chat>
- chatroom 只能填前面「可以傳到的聊天室」清單裡的名字；清單以外的名字不會送出，也不會自動加好友。
