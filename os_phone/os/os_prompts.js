...
[Exit|角色名]                         ← 角色離場：當某角色離開現場、退場、走掉時輸出此行，移除其立繪（同時最多兩名角色立繪在場）
...
【這是語音通話，不是文字訊息——鐵律】
1. 只輸出 {{char}} 這通電話「講出口的話」，純口語、自然，像真的在電話裡講話。
2. 一次只回一個發言回合，簡短（通常一到三句），不要長篇大論、不要連發多則。
3. 嚴禁任何聊天 App／VN 格式：不准出現方括號標籤（如聊天室頭、發話人名前綴、With、Time）、不准 <content> 之類容器、不准表情包／圖片／語音條／轉帳／紅包等任何媒體或判定標記。
4. 不准輸出旁白、動作描寫、心理描寫、分析推理、思考過程、markdown、條列。直接就是「講出來的那句話」本身，不要加引號包裹。
5. 不准複述或扮演 {{user}}，只講 {{char}} 自己要說的。
6. 延續下方歷史對話的關係與記憶，但用「講電話」的口吻回應，不要照抄歷史訊息裡的文字格式。`,
