...
(function() {
    ...
    const appStyle = `
        ...
        /* 組件 */
        .set-group { background: #1e1e1e; border-radius: 12px; padding: 20px; margin-bottom: 20px; display: flex; flex-direction: column; gap: 15px; border: 1px solid #333; }
        .set-label { font-size: 14px; color: #e2e8f0; margin-bottom: 6px; display: flex; justify-content: space-between; align-items: center; font-weight: 600; }
        .set-desc { font-size: 12px; color: #94a3b8; line-height: 1.5; margin-bottom: 5px; }
        .set-input, .set-select, .set-textarea { background: #2a2a2a; border: 1px solid #444; color: #fff; padding: 12px; border-radius: 8px; font-size: 14px; outline: none; width: 100%; box-sizing: border-box; transition: 0.2s; }
        .set-input:focus, .set-textarea:focus { border-color: #a855f7; background: #333; }
        .set-textarea { resize: vertical; min-height: 80px; font-family: monospace; line-height: 1.4; }
        
        /* 滑桿 */
        .set-slider-container { display: flex; flex-direction: column; gap: 8px; margin-bottom: 10px; }
        .set-slider { -webkit-appearance: none; width: 100%; height: 6px; background: #444; border-radius: 3px; outline: none; }
        .set-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 18px; height: 18px; background: #a855f7; border-radius: 50%; cursor: pointer; transition: 0.2s; }
        .set-slider::-webkit-slider-thumb:hover { transform: scale(1.2); }
        .set-slider-val { font-family: monospace; color: #a855f7; font-weight: bold; }

        /* 按鈕與開關 */
        .model-row { display: flex; gap: 10px; align-items: center; }
        .btn-fetch { background: #333; color: #fff; border: 1px solid #555; width: 44px; height: 42px; border-radius: 8px; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 20px; flex-shrink: 0; }
        .btn-save { padding: 14px; border-radius: 30px; text-align: center; font-weight: bold; cursor: pointer; margin-top: 10px; background: linear-gradient(135deg, #7c3aed 0%, #a855f7 100%); color: white; user-select: none; box-shadow: 0 4px 15px rgba(124, 58, 237, 0.4); transition: 0.2s; }
        .btn-test { padding: 12px; border-radius: 8px; text-align: center; font-weight: bold; cursor: pointer; margin-top: 10px; background: rgba(168, 85, 247, 0.1); color: #d8b4fe; border: 1px solid rgba(168, 85, 247, 0.3); }
        ...
        .set-status { font-size: 12px; color: #888; text-align: center; margin-top: 20px; min-height: 20px; padding-bottom: 20px; }
    ...
    const SEC_LLM_STORAGE_KEY = 'os_secondary_llm_config'; // 🔥 新增：副模型 Key
    const IMG_STORAGE_KEY = 'os_image_config';
    const MINIMAX_STORAGE_KEY = 'os_minimax_config';       // 🔥 新增：Minimax 語音設定
    ...
    function loadImageConfig() {
        ...
        if (saved) {
            try {
                ...
                // 🔄 舊版 defaultPrompt 自動遷移到 charBasePrompt
    ...
    function saveConfig(llmData, secLlmData, imgData, minimaxData) {
        ...
        localStorage.setItem(SEC_LLM_STORAGE_KEY, JSON.stringify(secLlmData)); // 🔥 保存副模型
    ...
    window.OS_SETTINGS = {
        ...
        getMinimaxConfig: loadMinimaxConfig,   // 🔥 新增：Minimax 語音設定
    ...
    function launchApp(container) {
        ...
        const secLlmConfig = loadSecLlmConfig(); // 🔥 載入副模型配置
        const imgConfig = loadImageConfig();
        const minimaxConfig = loadMinimaxConfig(); // 🔥 載入 Minimax 語音配置

        // 獨立模式偵測：在 index.html 中隱藏所有 ST 專屬 UI
        const isStandalone = !!(window.OS_API && typeof window.OS_API.isStandalone === 'function' && window.OS_API.isStandalone());
        // 獨立模式下強制以手動 API 模式渲染（避免 model-row 被隱藏）
        if (isStandalone) {
        ...
        // 構建 Profile 選項；若 currentId 為空（未曾存過），自動 fallback 到 ST 當前激活的設定檔
                ...
                <div class="set-content">
                    ...
                    <div id="view-llm" class="tab-view active">
                        <div class="set-group"${stHide}>
                            ...
                            <div id="st-profile-group" class="${llmConfig.useSystemApi ? '' : 'hidden'}" style="margin-top:10px; border-top:1px solid #333; padding-top:10px;">
                                <div class="set-label">選擇連接預設 (Profile)</div>
                                <select class="set-select" id="os-st-profile">${primaryProfileOpts}</select>
                                <div id="st-profile-info" style="margin-top:6px; font-size:11px; color:#888; word-break:break-all; line-height:1.6;"></div>
                            </div>
                        </div>
                        ...
                        <div class="set-group">
                            ...
                            <div id="model-system-notice" class="${llmConfig.useSystemApi ? '' : 'hidden'}" style="background:rgba(168,85,247,0.1); padding:10px; border-radius:8px; font-size:13px; color:#d8b4fe; border:1px solid rgba(168,85,247,0.3);">
                        ...
                        <div class="set-group">
                            ...
                            <div style="font-size:11px; color:#666; margin-top:6px;">讓模型回傳思考過程（需模型支援，如 Gemini 2.5 / Claude 3.5+）。開啟後 Temperature 自動設為 1。</div>
                        ...
                        <div class="set-group">
                            ...
                            <div id="os-test-result" style="display:none; margin-top:10px; background:#2a2a2a; border-radius:8px; padding:12px; font-size:12px; color:#ccc; font-family:monospace; white-space:pre-wrap; word-break:break-all; max-height:120px; overflow-y:auto;"></div>
                        </div>
                    </div>

                    <div id="view-sec-llm" class="tab-view hidden">
                        <div style="background:rgba(168, 85, 247, 0.1); padding:10px; border-radius:8px; margin-bottom:15px; border:1px solid rgba(168, 85, 247, 0.3); font-size:12px; color:#d8b4fe;">
                        ...
                        <div class="set-group"${stHide}>
                            ...
                            <div id="sec-st-profile-group" class="${secLlmConfig.useSystemApi ? '' : 'hidden'}" style="margin-top:10px; border-top:1px solid #333; padding-top:10px;">
                                <div class="set-label">選擇連接預設 (Profile)</div>
                                <select class="set-select" id="sec-st-profile">${secondaryProfileOpts}</select>
                                <div id="sec-st-profile-info" style="margin-top:6px; font-size:11px; color:#888; word-break:break-all; line-height:1.6;"></div>
                            </div>
                        </div>

                        <div class="set-group" id="sec-manual-api-group">
                        ...
                        <div class="set-group">
                            <div class="set-label">選擇模型</div>
                            <div id="sec-model-system-notice" class="${secLlmConfig.useSystemApi ? '' : 'hidden'}" style="background:rgba(168,85,247,0.1); padding:10px; border-radius:8px; font-size:13px; color:#d8b4fe; border:1px solid rgba(168,85,247,0.3);">
                                🔗 已跟隨酒館主系統，模型由酒館決定。<br>直接在酒館介面切換模型即可，無需在此設定。
                            </div>
                            <div class="model-row ${secLlmConfig.useSystemApi ? 'hidden' : ''}" id="sec-model-row">
                                ...
                                <div class="btn-fetch" id="sec-fetch-btn" title="${isStandalone ? '拉取模型清單' : '從酒館同步'}">${isStandalone ? '<i class="fa-solid fa-microchip"></i>' : '🔄'}</div>
                            </div>
                        </div>
                        ...
                        <div class="set-group">
                            ...
                            <div id="sec-test-result" style="display:none; margin-top:10px; background:#2a2a2a; border-radius:8px; padding:12px; font-size:12px; color:#ccc; font-family:monospace; white-space:pre-wrap; word-break:break-all; max-height:120px; overflow-y:auto;"></div>
                        </div>
                    </div>
                    ...
                    <div id="view-img" class="tab-view hidden">
                        <div class="set-group">
                            ...
                            <div id="img-group-pollinations" class="${imgConfig.service === 'pollinations' ? '' : 'hidden'}">
                                <div style="margin-top:15px;">
                                    <div class="set-label">API Key <span style="font-size:12px; color:#ff453a;">(必填 - 需儲值)</span></div>
                                    ...
                                    <div class="set-desc" style="color:#a855f7;">* 現在已無免費方案，請至官網獲取 Key。</div>
                                </div>
                                <div style="margin-top:15px;">
                            ...
                            <div id="img-group-nai" class="${imgConfig.service === 'novelai' ? '' : 'hidden'}">
                                <div style="margin-top:15px;">
                                    <div class="set-label">NovelAI Token <span style="font-size:12px; color:#ff453a;">(必填)</span></div>
                                ...
                                <div style="margin-top:15px; border:1px solid #333; border-radius:8px; overflow:hidden;">
                                    <div class="set-label" style="padding:10px 12px; color:#a855f7; cursor:pointer; user-select:none; display:flex; justify-content:space-between; align-items:center;"
                                        ...
                                        <span>⚙️ 進階參數 <span style="font-size:11px; color:#666; font-weight:normal;">（不懂可不動，預設跟官方一致）</span></span>
                                        <span class="nai-adv-arrow" style="font-size:11px; color:#666;">▶</span>
                                    </div>
                                    <div style="display:none; padding:12px; border-top:1px solid #333;">
                                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                                        ...
                                        <div>
                                            <div class="set-label">CFG Scale <span style="font-size:11px; color:#888;">（建議 4~9）</span></div>
                                            <input class="set-input" type="number" id="img-nai-scale" min="1" max="20" step="0.5" value="${imgConfig.novelai.scale ?? 5}">
                                        </div>
                                        <div>
                                            <div class="set-label">Steps <span style="font-size:11px; color:#888;">（建議 28~40）</span></div>
                                            <input class="set-input" type="number" id="img-nai-steps" min="1" max="50" step="1" value="${imgConfig.novelai.steps ?? 28}">
                                        </div>
                                        <div>
                                            <div class="set-label">UC Preset <span style="font-size:11px; color:#888;">（負詞預設集）</span></div>
                                    ...
                                    <div style="display:flex; gap:20px; margin-top:10px; flex-wrap:wrap;">
                                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;">
                                            ...
                                            <span class="set-label" style="margin:0;">Quality Toggle</span>
                                        </label>
                                        <label id="img-nai-smea-group" style="display:flex; align-items:center; gap:6px; cursor:pointer; ${imgConfig.novelai.model==='nai-diffusion-3'?'':'opacity:0.35; pointer-events:none;'}">
                                            ...
                                            <span class="set-label" style="margin:0;">SMEA <span style="font-size:11px; color:#888;">（V3 專屬，大幅提升品質）</span></span>
                                        </label>
                                        <label id="img-nai-smea-dyn-group" style="display:flex; align-items:center; gap:6px; cursor:pointer; ${imgConfig.novelai.model==='nai-diffusion-3'?'':'opacity:0.35; pointer-events:none;'}">
                                            ...
                                            <span class="set-label" style="margin:0;">SMEA Dynamic</span>
                                        </label>
                                    </div>
                                </div>
                                </div>
                                <div style="margin-top:15px; border:1px solid #3d2d5c; border-radius:8px; padding:12px; background:#110d1a;">
                                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px;">
                                        ...
                                        <span style="font-size:11px; color:#666;">含底詞、負詞、Sampler、Scale</span>
                                    </div>
                                    <div style="display:flex; gap:6px; align-items:center;">
                                        ...
                                        <span style="font-size:12px; color:#a855f7; cursor:pointer; white-space:nowrap; padding:4px 8px; border:1px solid #a855f7; border-radius:4px;" onclick="window._naiPreset.apply()">套用</span>
                                        <span style="font-size:12px; color:#a855f7; cursor:pointer; white-space:nowrap; padding:4px 8px; border:1px solid #a855f7; border-radius:4px;" onclick="window._naiPreset.save()">另存</span>
                                        <span style="font-size:12px; color:#ff6b6b; cursor:pointer; white-space:nowrap; padding:4px 8px; border:1px solid #ff6b6b; border-radius:4px;" onclick="window._naiPreset.del()">刪除</span>
                                    </div>
                                    <div id="img-nai-preset-name-row" style="display:none; margin-top:8px;">
                                        <div style="display:flex; gap:8px;">
                                            ...
                                            <span style="font-size:12px; color:#a855f7; cursor:pointer; padding:4px 8px; border:1px solid #a855f7; border-radius:4px; white-space:nowrap;" onclick="window._naiPreset.confirmSave()">確認</span>
                                            <span style="font-size:12px; color:#888; cursor:pointer; padding:4px 8px; border:1px solid #555; border-radius:4px;" onclick="window._naiPreset.cancelSave()">取消</span>
                                        </div>
                                    </div>
                                </div>

                                <div style="margin-top:15px;">
                                    <div class="set-label">🎨 角色底詞 <span style="font-size:11px; color:#888; font-weight:normal;">（Danbooru tag 格式，逗號分隔）</span></div>
                                    <textarea class="set-textarea" id="img-nai-char-base">${imgConfig.novelai.charBasePrompt || ''}</textarea>
                                    <div style="text-align:right; margin-top:4px;">
                                        <span style="font-size:11px; color:#a855f7; cursor:pointer;" onclick="document.getElementById('img-nai-char-base').value='masterpiece, best quality, very aesthetic, absurdres, anime style, detailed face'">[重置]</span>
                                    </div>
                                </div>
                                <div style="margin-top:15px;">
                                    ...
                                    <div style="text-align:right; margin-top:4px;">
                                        <span style="font-size:11px; color:#a855f7; cursor:pointer;" onclick="document.getElementById('img-nai-char-neg').value='nsfw, lowres, bad anatomy, bad hands, extra fingers, missing fingers, worst quality, low quality, jpeg artifacts, signature, watermark, blurry'">[重置]</span>
                                    </div>
                                </div>
                                <div style="margin-top:15px;">
                                    ...
                                    <div style="text-align:right; margin-top:4px;">
                                        <span style="font-size:11px; color:#a855f7; cursor:pointer;" onclick="document.getElementById('img-nai-item-base').value='masterpiece, best quality, white background, simple background, no background, product image, detailed'">[重置]</span>
                                    </div>
                                </div>
                                <div style="margin-top:15px;">
                                    ...
                                    <div style="text-align:right; margin-top:4px;">
                                        <span style="font-size:11px; color:#a855f7; cursor:pointer;" onclick="document.getElementById('img-nai-item-neg').value='person, human, character, body, face, hands, worst quality, low quality, blurry, watermark, text'">[重置]</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="set-group">
                            <div class="set-label">🎨 角色頭像通用底詞 <span style="font-size:10px; color:#888; font-weight:normal;">（全模組共用：夜店 / 偵探 / 寶寶 / VN頭像）</span></div>
                            <textarea class="set-textarea" id="img-style-prompt">${imgConfig.pollinations.charBasePrompt}</textarea>
                            <div style="font-size:10px; color:#666; margin-top:4px; line-height:1.5;">↑ 這裡設定的詞會自動加在所有角色頭像生圖的最前面。VN背景不受影響。VN頭像可在 VN 設定裡追加額外詞。</div>
                            <div style="text-align:right; margin-top:5px;">
                                <span style="font-size:11px; color:#a855f7; cursor:pointer;" onclick="document.getElementById('img-style-prompt').value='anime style, 2d, cel shading, flat color, illustration, high quality, best quality, no photorealistic, no 3d, clean lines'">[重置為預設]</span>
                            </div>
                        </div>
                        ...
                        <div class="set-group">
                            ...
                            <div style="text-align:right; margin-top:5px;">
                                <span style="font-size:11px; color:#a855f7; cursor:pointer;" onclick="document.getElementById('img-char-neg-prompt').value='bad anatomy, extra limbs, disfigured, blurry, low quality, worst quality, watermark, text'">[重置為預設]</span>
                            </div>
                        </div>

                        <div class="set-group">
                            <div class="set-label">🐾 寵物圖片底詞 <span style="font-size:10px; color:#888; font-weight:normal;">（寵物商店 / 合成）</span></div>
                            <textarea class="set-textarea" id="img-pet-prompt">${imgConfig.pollinations.petBasePrompt}</textarea>
                            <div style="font-size:10px; color:#666; margin-top:4px; line-height:1.5;">↑ 寵物專用，不影響人物頭像。角色底詞與寵物底詞完全分離。</div>
                            <div style="text-align:right; margin-top:5px;">
                                <span style="font-size:11px; color:#a855f7; cursor:pointer;" onclick="document.getElementById('img-pet-prompt').value='cute, 2D art, sticker style, simple background, white background, high quality'">[重置為預設]</span>
                            </div>
                        </div>
                        ...
                        <div class="set-group">
                            ...
                            <div style="text-align:right; margin-top:5px;">
                                <span style="font-size:11px; color:#a855f7; cursor:pointer;" onclick="document.getElementById('img-pet-neg-prompt').value='bad anatomy, blurry, low quality, worst quality, human, person, watermark, text'">[重置為預設]</span>
                            </div>
                        </div>
                        </div>
                        ...
                        <div class="set-group">
                            ...
                            <div id="img-test-preview" style="margin-top:15px; display:none; text-align:center;">
                                <img id="img-test-image" style="max-width:100%; border-radius:8px;" />
                                <div id="img-test-url" style="font-size:10px; color:#666; margin-top:5px;"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="view-voice" class="tab-view hidden">
                        ...
                        <div style="background:rgba(99,179,237,0.08); padding:10px; border-radius:8px; margin-bottom:15px; border:1px solid rgba(99,179,237,0.3); font-size:12px; color:#90cdf4;">
                        ...
                        <div class="set-group">
                            <div class="set-label">Group ID <span style="font-size:12px; color:#ff453a;">(必填)</span></div>
                        ...
                        <div class="set-group">
                            <div class="set-label">API Key <span style="font-size:12px; color:#ff453a;">(必填)</span></div>
                        ...
                        <div class="set-group">
                            ...
                            <div style="display:flex; gap:8px; margin-top:12px;">
                                ...
                                <div class="btn-test" id="mm-browse-voices-btn" style="flex:1; background:#2d1f44;">🔍 瀏覽官方音色庫</div>
                            </div>
                        </div>
                        ...
                        <div id="mm-voice-modal" style="display:none; position:fixed; inset:0; z-index:9999; background:rgba(0,0,0,0.75); align-items:center; justify-content:center;">
                            <div style="background:#1a1225; border:1px solid #6b37a0; border-radius:12px; padding:16px; width:92%; max-width:480px; max-height:82vh; display:flex; flex-direction:column; box-shadow:0 8px 32px rgba(0,0,0,0.6);">
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                                    <div style="font-weight:bold; color:#d8b4fe; font-size:14px;">🎵 官方音色庫 <span id="mm-voice-count" style="font-size:11px; color:#888; font-weight:normal;"></span></div>
                                    <span id="mm-voice-modal-close" style="cursor:pointer; color:#888; font-size:20px; line-height:1; padding:0 4px;">✕</span>
                                </div>
                                <input id="mm-voice-search" class="set-input" placeholder="搜尋 voice_id 或描述..." style="margin-bottom:8px;">
                                <div id="mm-voice-list" style="overflow-y:auto; flex:1; display:flex; flex-direction:column; gap:6px;"></div>
                            </div>
                        </div>
                        ...
                        <div class="set-group">
                            ...
                            <div id="mm-test-result" style="display:none; margin-top:10px; background:#2a2a2a; border-radius:8px; padding:12px; font-size:12px; color:#ccc; font-family:monospace; word-break:break-all;"></div>
                        </div>
                    </div>
                    ...
                    <div id="view-sys" class="tab-view hidden">
                        ...
                        <div class="set-group">
                            ...
                            <div id="bk-gist-hint" style="font-size:11px; color:#a855f7; margin-top:6px; word-break:break-all;"></div>
                        ...
                        <div class="set-group">
                            <div class="set-label">
                                📊 本地儲存空間
                                <span class="btn-test" id="bk-scan-btn" style="padding:4px 12px; font-size:11px; cursor:pointer;">掃描</span>
                            </div>
                        ...
                        <div id="bk-status" style="font-size:12px; color:#888; text-align:center; padding:10px 0; min-height:20px;"></div>
                    </div>
        ...
        const secValTokens = container.querySelector('#sec-val-tokens');
        ...
        const secFetch = container.querySelector('#sec-fetch-btn'); // 🔥 副模型 Fetch
        ...
        // Sliders Listeners
        // Max Tokens 已改為 number input，不需要同步顯示值
        ...
        // sec Max Tokens 已改為 number input，不需要同步顯示值
        ...
        const toggleInputs = (sysApi, manGroup, profGroup) => {
            if (sysApi.checked) { manGroup.classList.add('hidden'); profGroup.classList.remove('hidden'); } 
            else { manGroup.classList.remove('hidden'); profGroup.classList.add('hidden'); }
        };
        ...
        function showProfileInfo(selectEl, infoEl) {
            ...
            if (!selectedId) {
                infoEl.innerHTML = '<span style="color:#666">ℹ️ 使用當前激活的 ST 連接</span>';
                return;
            }
            ...
            const api = p.api ? ` <span style="color:#555">[${p.api}]</span>` : '';
        ...
        btnSave.onclick = () => {
            try {
                ...
                const secLlmData = {
                    ...
                    url: secUrl.value.trim(),
                    key: secKey.value.trim(),
                ...
                status.innerText = `❌ 保存失敗: ${e.message}`; status.style.color = "#ff453a";
            }
        };
        ...
        async function runApiTest(cfg, modelVal, resultEl) {
            ...
            try {
                ...
                status.innerText = '✅ API 連線成功'; status.style.color = '#07c160';
            } catch (e) {
                resultEl.style.color = '#ff453a';
                resultEl.textContent = '❌ 錯誤: ' + e.message;
                status.innerText = '❌ API 測試失敗'; status.style.color = '#ff453a';
            }
        }
        ...
        secTestBtn.onclick = async () => {
            ...
            await runApiTest(
                { useSystemApi: secSystemApi.checked, stProfileId: secStProfile.value, url: secUrl.value.trim(), key: secKey.value.trim() },
                secModel.value,
        ...
        // --- GPT-SoVITS 開關（即時寫入，不走 saveConfig）---
        ...
        window._naiPreset = {
            apply() {
                ...
                const setChk = (id, v) => { const el = container.querySelector(id); if (el) el.checked = !!v; };
            ...
            confirmSave() {
                ...
                console.log(`[NAI預設] 已新增「${name}」，共 ${naiPresets.length} 個`);
            },
            cancelSave() {
                const row = container.querySelector('#img-nai-preset-name-row');
                if (row) row.style.display = 'none';
        ...
        function makeAliasChip(alias) {
            ...
            chip.style.cssText = 'display:inline-flex; align-items:center; gap:4px; background:#3a2a4a; border:1px solid #6b37a0; border-radius:20px; padding:3px 10px; font-size:12px; color:#d8b4fe; margin:2px;';
            chip.innerHTML = `${alias} <span style="cursor:pointer; color:#a855f7; font-size:14px; line-height:1;" title="移除">×</span>`;
        ...
        function makeProfileCard(profile = {}, expanded = false) {
            ...
            card.style.cssText = 'background:#1a1a2a; border:1px solid #3a2a4a; border-radius:10px; overflow:hidden;';

            // --- 標題列（永遠可見，點擊展開/收合）---
            const header = document.createElement('div');
            header.style.cssText = 'display:flex; align-items:center; gap:8px; padding:10px 14px; cursor:pointer; user-select:none;';
            header.innerHTML = `
                <span class="mm-p-arrow" style="color:#888; font-size:12px; transition:transform 0.2s; flex-shrink:0;">${expanded ? '▼' : '▶'}</span>
                <span class="mm-p-name-display" style="flex:1; font-weight:600; color:#d8b4fe; font-size:14px;">${profile.label || '（未命名）'}</span>
                <span style="flex-shrink:0; cursor:pointer; color:#ff453a; font-size:18px; padding:2px 4px;" title="刪除此音色">🗑</span>
            `;
            ...
            nameRow.innerHTML = `<div class="set-label" style="font-size:12px; margin-bottom:4px;">角色名稱</div>
                <input class="set-input mm-p-label" type="text" placeholder="如：愛麗絲" value="${profile.label || ''}" style="font-weight:600; color:#d8b4fe;">`;
            // 同步更新標題顯示名
            ...
            aliasSection.innerHTML = `<div class="set-label" style="font-size:12px; margin-bottom:6px;">別名 <span style="color:#888; font-weight:normal;">（大小寫不敏感）</span></div>`;
            const chipContainer = document.createElement('div');
            chipContainer.style.cssText = 'display:flex; flex-wrap:wrap; gap:2px; min-height:28px; background:#111; border:1px solid #333; border-radius:8px; padding:6px; margin-bottom:6px;';
        ...
        function renderVoiceList(voices) {
            ...
            voices.forEach(v => {
                ...
                row.style.cssText = 'display:flex; align-items:flex-start; gap:8px; background:#110d1a; border:1px solid #3d2d5c; border-radius:8px; padding:10px 12px; cursor:default;';
                const desc = Array.isArray(v.description) ? v.description[0] : (v.description || '');
                row.innerHTML = `
                    <div style="flex:1; min-width:0;">
                        <div style="font-size:13px; color:#e2d4f8;">${desc || v.voice_id}</div>
                        <div style="font-size:10px; color:#6b5a7a; font-family:monospace; margin-top:3px; word-break:break-all;">${v.voice_id}</div>
                    </div>
                    <div style="display:flex; flex-direction:column; gap:4px; flex-shrink:0;">
                        <button class="mm-v-use"   style="font-size:11px; background:#3a2a4a; border:1px solid #6b37a0; color:#d8b4fe; border-radius:6px; padding:3px 8px; cursor:pointer; white-space:nowrap;">填入測試</button>
                        <button class="mm-v-add"   style="font-size:11px; background:#1a3a2a; border:1px solid #37a06b; color:#86efac; border-radius:6px; padding:3px 8px; cursor:pointer; white-space:nowrap;">新增檔案</button>
                    </div>`;
        ...
        if (mmBrowseBtn && mmVoiceModal) {
            mmBrowseBtn.onclick = async () => {
                ...
                mmVoiceList.innerHTML = '<div style="text-align:center; color:#888; padding:20px;">⏳ 載入中...</div>';
                mmVoiceSearch.value = '';
                ...
                try {
                    ...
                    mmVoiceList.innerHTML = `<div style="color:#ff453a; padding:10px;">❌ 載入失敗：${err.message}</div>`;
                }
            };
        ...
        if (mmTestBtn) {
            mmTestBtn.onclick = async () => {
                ...
                mmResult.style.display = 'block';
                mmResult.style.color = '#888';
                mmResult.textContent = '⏳ 正在呼叫 Minimax TTS API...';

                // 臨時更新 OS_MINIMAX 配置以使用表單中的值
                const win = window.parent || window;
                if (win.OS_MINIMAX) {
                    // 🔥 保留現有 voiceProfiles，只更新連線相關欄位，避免覆蓋音色設定檔
        ...
        if (mmStopBtn) {
            mmStopBtn.onclick = () => {
                ...
                if (mmResult) { mmResult.style.color = '#888'; mmResult.textContent = '已停止播放'; }
            };
        }
        ...
        btnImgTest.onclick = async () => {
            ...
            try {
                ...
                const imageUrl = await imageManager.generateBackgroundAsync(testPrompt, { width, height });

                imgTestImage.src = imageUrl;
                imgTestUrl.textContent = `URL: ${imageUrl}`;
                imgTestPreview.style.display = 'block';

                imgTestImage.onload = () => { status.innerText = "✅ 圖片加載成功"; status.style.color = "#07c160"; };
                imgTestImage.onerror = () => { status.innerText = "❌ 圖片加載失敗 (請檢查 API Key)"; status.style.color = "#ff453a"; };
    ...
    function bindBackupTab(container) {
        ...
        function setStatus(msg, color) {
            if (elStatus) { elStatus.textContent = msg; elStatus.style.color = color || '#888'; }
        }
        ...
        if (btnGistSave) btnGistSave.addEventListener('click', async () => {
            if (!BACKUP) { setStatus('❌ OS_BACKUP 模組未載入', '#ff453a'); return; }
            const token = elToken?.value.trim();
            const gistId = elGistId?.value.trim() || null;
            if (!token) { setStatus('請先填入 GitHub Token', '#ff453a'); return; }
            BACKUP.saveSettings({ token, gistId });
            setBtnLoading(btnGistSave, '備份中...');
            setStatus('⏳ 正在備份到 GitHub Gist...', '#a855f7');
            try {
                ...
                setStatus('✅ 備份完成（' + result.sizeKB + ' KB）', '#07c160');
            } catch(e) { setStatus('❌ 備份失敗：' + e.message, '#ff453a'); }
        ...
        if (btnGistRestore) btnGistRestore.addEventListener('click', async () => {
            ...
            const token = elToken?.value.trim();
            const gistId = elGistId?.value.trim() || null;
            if (!token || !gistId) { setStatus('請先填入 Token 與 Gist ID', '#ff453a'); return; }
            if (!confirm('從 Gist 還原將合併資料（不清空現有），確定繼續？')) return;
            BACKUP.saveSettings({ token, gistId });
            setBtnLoading(btnGistRestore, '還原中...');
            setStatus('⏳ 正在從 GitHub Gist 還原...', '#a855f7');
            try {
                const data = await BACKUP.gistRestore();
                const result = await BACKUP.applyData(data);
                setStatus(`✅ 還原完成：世界書 ${result.worldbook} 條、寵物 ${result.pets} 隻、設定 ${result.localStorage} 項`, '#07c160');
            } catch(e) { setStatus('❌ 還原失敗：' + e.message, '#ff453a'); }
            ...
            try {
                ...
                const lines = Object.entries(info).map(([k, v]) =>
                    `<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #2a2a2a;">
                        ...
                        <span style="color:#a855f7">${v.count} 筆 / ${v.kb} KB</span>
                    </div>`
                ).join('');
        ...
        if (btnExport) btnExport.addEventListener('click', async () => {
            ...
            setStatus('⏳ 正在打包資料...', '#a855f7');
            try {
                const sizeKB = await BACKUP.exportLocal();
                setStatus('✅ 匯出完成（' + sizeKB + ' KB），請儲存至安全位置', '#07c160');
            } catch(e) { setStatus('❌ 匯出失敗：' + e.message, '#ff453a'); }
        ...
        if (fileInput) fileInput.addEventListener('change', async (e) => {
            ...
            setStatus('⏳ 正在匯入...', '#a855f7');
            try {
                const result = await BACKUP.importLocal(file);
                setStatus(`✅ 匯入完成：世界書 ${result.worldbook} 條、寵物 ${result.pets} 隻、設定 ${result.localStorage} 項`, '#07c160');
            } catch(e) { setStatus('❌ 匯入失敗：' + e.message, '#ff453a'); }
            e.target.value = '';
        });
    }
