...
(function () {
    ...
    const LOG_MAX = 20;
    // 小批次送：Jev 回 503「暫時無法服務」有兩種——它當下太忙（時好時壞），跟一通太大（穩定被拒）。
    //   官方（TypeSafe、Vercel）都沒寫題數上限，只寫一題最多 255 個選項、一次約 64000 token；
    //   但 09-23 實測整場 30 題×110 個選項一通送，連 8 次被拒，切成一通 3 題就過。所以照實測切小批，不照文件。
    const Q_PER_CALL = 12;
