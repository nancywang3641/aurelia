...
(function () {
    ...
    //   09-26 她：「那人去上廁所，就誤以為同一個小房間」。都市恶宴第 72 章罗德進宿舍洗手間洗澡，隔 60 段才又開口，
    //   舊題目只說「不在這個場景裡」，Jev 答「一直都在」→ 立繪掛到他洗完出來。題目補「走到看不見的地方就不算在場，
    //   就算只是同一間屋子的另一個房間；只聽得到聲音也不算」之後，最晚在鏡頭離開房間時收掉；原本四章沒有多收錯。
    //   試過「照段序累加機率、過 30%/40% 就收」：罗德會在剛進洗手間就收，但咖啡店那章多收錯兩個還在場的人，沒採用。
    function _questions(ss) {
        ...
        ss.forEach((s, i) => {
            ...
            qs.push({
                id: 'q' + s.p, name: s.who,
                q: { type: 'choice', criteria, instructions: '「' + s.who + '」在 P' + s.p + ' 說完話之後，到' + end + '之前，他是從第幾段起就不在這個場景裡了？（人走到這個畫面看不見的地方就不算在場——就算只是走進同一間屋子的另一個房間；講電話、傳訊息、只被別人提到、只聽得到聲音、回憶裡出現，也都不算在場；如果中間一直都在，選「' + STAY + '」）' },
            });
        });
        return qs;
    }
