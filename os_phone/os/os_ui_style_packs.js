...
(function () {
    ...
    // 🚨每條都要帶一個「看得出有沒有做到」的幾何關係，不能只有意象。
    //   只寫「大面積斜切與對角動線」，模型會加一條 skew 的裝飾帶交差，底下照樣是縱向堆疊——
    //   形容詞管不到區塊的數量、大小與位置，只有關係才管得到。
    const LAYOUTS = {
        L1: { name: 'ticket-rail',        desc: '票根與票面：一條裁切線把畫面切成不等寬的兩塊，窄的那條從頭貫穿到尾，序號、條碼與刻度長在它身上' },
        L2: { name: 'diagonal-surge',     desc: '對角動線：至少一條主要分界不是水平也不是垂直的，主視覺跨過那條斜邊、壓進兩側區塊' },
        L3: { name: 'object-interface',   desc: '整個介面是一件物品（卡帶、卷宗、護照、終端、儀表）：外殼本身就是邊界，資料嵌在它原有的螢幕、凹槽或窗口裡，不可以有獨立於外殼之外的卡片' },
        L4: { name: 'editorial-bleed',    desc: '編輯出血：至少一個元素（巨大字、色塊或圖像）被畫面邊緣裁掉一截，其餘靠共用基線與留白維持秩序' },
        L5: { name: 'bento-dashboard',    desc: '資訊磚：格子至少三種不同尺寸，最大那格的面積至少是最小的四倍，並且要有格子跨欄或跨列' },
        L6: { name: 'evidence-desk',      desc: '桌面卷宗：文件、照片、標籤像被人擺上桌，彼此可以互相壓住一角，不必對齊同一條線' },
        L7: { name: 'scrapbook-pinboard', desc: '拼貼板：沒有任何兩個元素共用同一條對齊線，紙片帶輕微角度，膠帶或圖釘壓在交界處' },
        L8: { name: 'specimen-showroom',  desc: '標本展示：主體佔畫面一半以上並置中，規格資料貼著四周邊緣走，中間大量留白' },
        L9: { name: 'layered-tabs',       desc: '分層抽屜：同一塊區域輪流承載多組資料，靠標籤或頁籤切換，不把全部攤在同一頁' },
        L10:{ name: 'poster-impact',      desc: '海報：一個巨大的標題、角色或符號佔掉畫面三分之一以上，其餘資料退成沿邊的微型標註' },
    };
    ...
    // 🚨變奏只能調「骨架之內」的東西。以前這裡有一組 flow（資訊走 Z 字／上下分層／環繞主體），
    //   那是跟骨架同層級的決定 —— 骨架說「格子要跨欄」、變奏說「資訊上下分層」，兩道指令互相抵消，
    //   模型收到打架的要求就退回它最熟的縱向堆疊。同層級的話只能有一個人講。
    const VARIANTS = {
    ...
    const PRINCIPLES =
        '【共用美術原則】\n' +
        // 🚨隱喻以前只接到顏色，接不到骨架 —— 那件東西的構造才是版面，只拿它挑色票等於換皮不換骨
        '- 先想一句話說得出的視覺隱喻(這個面板「是什麼東西」)，讓它先決定版面怎麼分塊，再決定顏色。\n' +
        '  隱喻只用來挑配色的話，畫面照樣是同一副骨架換一層皮——那件東西本身分成幾塊、哪塊大、哪塊嵌在哪，才是版面。\n' +
    ...
    function build(styleId, layoutId, opts) {
        ...
        const variant = [_rand(VARIANTS.focus), _rand(VARIANTS.density),
        ...
        let s = PRINCIPLES + '\n【本次美術方向:' + p.name + '】\n' +
            ...
            '　' + p.type + '\n' +
            '　' + p.detail + '\n' +
            // 🚨這行以前寫「(不是非這樣不可)」——整段裡唯一講骨架的一句話自帶免責，
            //   而旁邊的材質、邊框、裝飾全是肯定句又具體到能直接寫成 HTML。
            //   結果就是皮每次都換、骨永遠不換。骨架這件事不能給退路，該給的自由在下面那段。
            '這次的骨架(照這個走,不要換成縱向堆疊):' + L.desc + '。\n' +
            // comp 是這包的比例慣例，不是第二副骨架：像 frontier-ticket 的「側邊票根」配 L1 沒問題，
            //   配 L4(出血裁切)就矛盾。兩個骨架級指令並排＝互相抵消，所以要標明誰說了算。
            '　 這個風格慣用的比例與重心:' + p.comp + '(跟骨架牴觸時以骨架為準)。\n' +
            '配色結構:' + p.color + '。\n';
        if (opts.cssOnly) {
