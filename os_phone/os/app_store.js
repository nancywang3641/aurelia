...
(function () {
    ...
    function _wsPrompt(desc, provider) {
        return '你是資深前端工程師。請依需求設計一個「功能型 HTML 小 app」，把它拆成 css / html / js 三段輸出。\n\n'
            ...
            + '- callAI(systemPrompt) → Promise<string>：呼叫 AI 生成文字、回純文字。它「會」自動帶上當前角色卡(角色描述/個性/情境)與最近劇情，但「不會」吃酒館預設與全域世界書(所以會乖乖照你的 systemPrompt 走)。把這個 app 的具體要求與輸出格式寫進 systemPrompt 即可、不必自己重述角色設定。\n'
    ...
    function _assembleApp(parsed, provider) {
        ...
        return '<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8">\n'
            ...
            + '<scr' + 'ipt>(function(){\n'
            + 'async function callAI(sys){ try { var r = await window.generateRaw({ ordered_prompts:["char_description","char_personality","scenario","chat_history",{role:"system",content:sys}], max_chat_history:30 }); return typeof r==="string"?r:(r&&r.message)||""; } catch(e){ console.error("[app callAI]",e); return ""; } }\n'
