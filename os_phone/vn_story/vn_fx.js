...
(function () {
    ...
    const BUILTINS = [
        ...
        { fxId: 'fx-cctv', name: '監視器畫面', desc: '監控鏡頭訊號感：掃描線＋雜訊雪花＋滾動亮帶＋REC紅點（回憶錄影/監控視角/偷窺/詭異氛圍），出現約 6 秒後淡掉', use: '監控視角、偷窺、回憶錄影', kind: 'once', steps: [
            { block: 'code', at: 0, dur: 6000, js: [
                'if(!state.init){state.init=1;state.roll=-40;state.t=0;}',
                'state.t+=dt;',
                'var a=p<0.12?p/0.12:(p>0.75?(1-p)/0.25:1);',
                'ctx.globalAlpha=0.16*a;ctx.fillStyle="#0a140c";ctx.fillRect(0,0,w,h);',
                'var g=ctx.createRadialGradient(w/2,h/2,Math.min(w,h)*0.35,w/2,h/2,Math.max(w,h)*0.72);g.addColorStop(0,"rgba(0,0,0,0)");g.addColorStop(1,"#000000");ctx.globalAlpha=0.375*a;ctx.fillStyle=g;ctx.fillRect(0,0,w,h);',
                'ctx.fillStyle="#000000";ctx.globalAlpha=0.16*a;',
                'for(var y=0;y<h;y+=3)ctx.fillRect(0,y,w,1);',
                'ctx.fillStyle="#cfd8cf";',
                'for(var i=0;i<80;i++){ctx.globalAlpha=(0.06+Math.random()*0.22)*a;ctx.fillRect(Math.random()*w,Math.random()*h,1.5,1.5);}',
                'state.roll+=dt*h*0.25;if(state.roll>h+40)state.roll=-40;',
                'ctx.globalAlpha=0.06*a;ctx.fillStyle="#ffffff";ctx.fillRect(0,state.roll,w,26);',
                'if(Math.random()<0.02){ctx.globalAlpha=0.22*a;ctx.fillStyle="#9fb3a3";ctx.fillRect(0,Math.random()*h,w,2+Math.random()*5);}',
                'if(Math.floor(state.t*1.4)%2===0){ctx.globalAlpha=0.9*a;ctx.fillStyle="#e03434";ctx.beginPath();ctx.arc(26,24,6,0,6.29);ctx.fill();}',
                'ctx.globalAlpha=1;',
            ].join('\n') },
        ]},
        { fxId: 'fx-wind', name: '風吹', desc: '氣流線橫掃＋落葉翻滾橫飛＋風聲（狂風/山頂曠野/離別氛圍/風系魔法），持續到換場', use: '狂風、曠野、離別、風系魔法', kind: 'loop', sfx: 'weather-wind', steps: [
            { block: 'code', js: [
                ...
                ' var cols=["#9db85a","#c2a24b","#7fa05a","#b58b3e"];',
                ' for(var j=0;j<10;j++)state.leaves.push({x:Math.random()*w,y:Math.random()*h,sp:240+Math.random()*260,ph:Math.random()*6.28,rot:Math.random()*6.28,vr:2+Math.random()*3.5,s:4+Math.random()*5,c:cols[j%4]});}',
                'ctx.lineCap="round";',
                'for(var k=0;k<state.streaks.length;k++){var st=state.streaks[k];st.x+=st.sp*dt;st.y+=st.sp*dt*0.12;',
                ' if(st.x-st.len>w){st.x=-st.len-Math.random()*w*0.3;st.y=Math.random()*h;}',
                ' ctx.globalAlpha=st.a*p;ctx.strokeStyle="#ffffff";ctx.lineWidth=st.th;',
                ' ctx.beginPath();ctx.moveTo(st.x-st.len,st.y-st.len*0.12);ctx.lineTo(st.x,st.y);ctx.stroke();}',
                'for(var m=0;m<state.leaves.length;m++){var lf=state.leaves[m];lf.x+=lf.sp*dt;lf.ph+=dt*3;lf.y+=Math.sin(lf.ph)*42*dt+lf.sp*dt*0.1;lf.rot+=lf.vr*dt;',
                ' if(lf.x-20>w||lf.y-20>h){lf.x=-20-Math.random()*60;lf.y=Math.random()*h*0.8;}',
                ' ctx.globalAlpha=0.75*p;ctx.fillStyle=lf.c;ctx.save();ctx.translate(lf.x,lf.y);ctx.rotate(lf.rot);',
                ' ctx.beginPath();ctx.ellipse(0,0,lf.s,lf.s*0.45,0,0,6.29);ctx.fill();ctx.restore();}',
                'ctx.globalAlpha=1;',
            ].join('\n') },
        ]},
