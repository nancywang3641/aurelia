...
async function handleMessageEvent(event) {
    ...
    if (event.data && (event.data.type === 'CHAT_DATA' || event.data.type === 'CHAT_DATA_STREAM')) {
        console.log('🔥 [聊天面板] 收到CHAT_DATA消息:', {
            type: event.data.type,
            hasData: !!event.data.data,
            dataKeys: event.data.data ? Object.keys(event.data.data) : [],
            chatListLength: event.data.data?.chatList?.length || 0,
        ...
        // 🔥 新增：如果还没初始化，先初始化
        if (!isInitialized) {
            console.log('🔥 [聊天面板] 收到数据但未初始化，先初始化...');
            delayedInit();
            // 延迟处理数据
            setTimeout(() => {
                handleReceivedChatData(event.data.data, event.data.dataSource || 'AUTO', event.data.forceRefresh);
            }, 1000);
            return;
        }
