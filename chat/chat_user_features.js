...
function findTavernMainWindow() {
    try {
        ...
        // 首先嘗試使用跨域通信
        if (window.CrossOriginComm && window.CrossOriginComm.isBridgeConnected()) {
            console.log('[时间调节] ✅ 使用跨域通信橋接器');
            return {
                type: 'cross-origin',
                comm: window.CrossOriginComm,
                sendMessage: async (message) => {
                    return await window.CrossOriginComm.sendMessageToTavern(message);
                }
            };
        }
        ...
        while (currentWindow.parent && currentWindow.parent !== currentWindow && depth < 5) {
            ...
            if (currentWindow.document && 
                ...
                return {
                    ...
                    sendMessage: (message) => {
                        const textarea = currentWindow.document.querySelector('#send_textarea');
                        const sendButton = currentWindow.document.querySelector('#send_but');
                        ...
                        return Promise.reject(new Error('找不到發送界面元素'));
                    }
                };
            }
        }
