    ...
    if (targetContainer) {
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                console.log(`[刪除聊天室] 已更新 localStorage 中的聊天數據: ${chatId}`);
            } catch (error) {
                console.error('[刪除聊天室] 更新 localStorage 失敗:', error);
            }
            ...
            try {
                ...
                // 🔥 重要：更新 localStorage 中的聊天狀態
                try {
                    localStorage.setItem('chat_message_states', JSON.stringify(chatMessageStates));
                    console.log(`[刪除聊天室] 已更新 localStorage 中的聊天狀態: ${chatId}`);
                } catch (error) {
                    console.error('[刪除聊天室] 更新聊天狀態 localStorage 失敗:', error);
                }
                
                // 🔥 重要：更新 localStorage 中的未讀狀態
                try {
                    localStorage.setItem('chat_unread_state', JSON.stringify(chatUnreadState));
                    console.log(`[刪除聊天室] 已更新 localStorage 中的未讀狀態: ${chatId}`);
                } catch (error) {
                    console.error('[刪除聊天室] 更新未讀狀態 localStorage 失敗:', error);
                }
                ...
                if (window.temporaryGroups) {
                    window.temporaryGroups = window.temporaryGroups.filter(group => group.id !== chatId);
                    console.log(`[刪除聊天室] 已清理臨時群組數據: ${chatId}`);
                }
                
                // 🔥 重要：清理臨時私聊數據
                if (window.temporaryPrivateChats) {
                    window.temporaryPrivateChats = window.temporaryPrivateChats.filter(chat => chat.id !== chatId);
                    console.log(`[刪除聊天室] 已清理臨時私聊數據: ${chatId}`);
                }
                ...
                if (window.transferTimerManager) {
                    try {
                        // 清理轉帳計時器
                        window.transferTimerManager.stopTimer(`transfer_${chatId}`);
                        // 清理禮物計時器
                        window.transferTimerManager.stopTimer(`gift_${chatId}`);
                        console.log(`[刪除聊天室] 已清理相關計時器: ${chatId}`);
                    } catch (error) {
                        console.warn('[刪除聊天室] 清理計時器失敗:', error);
                    }
                }
                ...
                try {
                    // 清理群組相關頭像數據
                    if (window.groupAvatarData) window.groupAvatarData = null;
                    if (window.adminAvatarData) window.adminAvatarData = null;
                    if (window.groupSettingsGroupAvatarData) window.groupSettingsGroupAvatarData = null;
                    if (window.groupSettingsAdminAvatarData) window.groupSettingsAdminAvatarData = null;
                    
                    // 清理私聊相關頭像數據
                    if (window.userAvatarData) window.userAvatarData = null;
                    if (window.characterAvatarData) window.characterAvatarData = null;
                    if (window.privateChatSettingsUserAvatarData) window.privateChatSettingsUserAvatarData = null;
                    if (window.privateChatSettingsCharacterAvatarData) window.privateChatSettingsCharacterAvatarData = null;
                    
                    console.log(`[刪除聊天室] 已清理相關頭像數據: ${chatId}`);
                } catch (error) {
                    console.warn('[刪除聊天室] 清理頭像數據失敗:', error);
                }
                
                // 🔥 重要：清理相關設置數據
                try {
                    // 清理群組設置相關數據
                    if (window.groupSettingsCurrentMembers) window.groupSettingsCurrentMembers = [];
                    if (window.isGroupSettingsMode) window.isGroupSettingsMode = false;
                    
                    console.log(`[刪除聊天室] 已清理相關設置數據: ${chatId}`);
                } catch (error) {
                    console.warn('[刪除聊天室] 清理設置數據失敗:', error);
                }
                
                // 🔥 重要：清理相關緩存
                try {
                    // 清理渲染器緩存
                    if (window.advancedRenderer && typeof window.advancedRenderer.clearCache === 'function') {
                        window.advancedRenderer.clearCache();
                    }
                    
                    // 清理API緩存
                    if (window.TavernAPI && typeof window.TavernAPI.clearCache === 'function') {
                        window.TavernAPI.clearCache();
                    }
                    
                    console.log(`[刪除聊天室] 已清理相關緩存: ${chatId}`);
                } catch (error) {
                    console.warn('[刪除聊天室] 清理緩存失敗:', error);
                }
